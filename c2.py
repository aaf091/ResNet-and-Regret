...
def getargs():
    ...
    parser.add_argument('--use_cuda', type=bool, default=True,
                        help='Whether to use CUDA for training. By Default it is set True, so it will try to use GPU if available else it will default to CPU')
...
def report_q3_counts(model: nn.Module, device: torch.device, num_classes: int = 10):
    """
    Q3 helper: counts trainable parameters and gradient tensors/elements.
    Runs a tiny forward/backward pass so .grad is populated.
    """
    # Count trainable parameter elements/tensors
    trainable_param_elements = sum(p.numel() for p in model.parameters() if p.requires_grad)
    trainable_param_tensors  = sum(1 for p in model.parameters() if p.requires_grad)

    # Tiny forward/backward so gradients exist
    model.train()
    ...
    out = model(x)
    loss = criterion_local(out, y)
    loss.backward()

    # Count gradient tensors/elements that actually materialized
    grad_tensors = 0
    grad_elements = 0
    for p in model.parameters():
        if p.requires_grad and p.grad is not None:
            grad_tensors += 1
            grad_elements += p.grad.numel()

    # (Optional) clear grads so training below starts clean
    for p in model.parameters():
        p.grad = None

    print("\n========== Q3: Parameter & Gradient Counts (SGD) ==========")
    print(f"Trainable parameter tensors: {trainable_param_tensors}")
    print(f"Trainable parameter elements: {trainable_param_elements}")
    print(f"Gradient tensors (after backward): {grad_tensors}")
    print(f"Gradient elements (after backward): {grad_elements}")
    print("===========================================================\n")

from contextlib import contextmanager

@contextmanager
def maybe_record(prof_enabled, name):
    if prof_enabled:
...
def main():
    ...
    report_q3_counts(net, device, num_classes=10)
